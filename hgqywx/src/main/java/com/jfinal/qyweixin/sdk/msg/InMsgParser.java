...
public class InMsgParser {
	...
	private static InMsg parseInEvent(XmlHelper xmlHelper, String toUserName, String fromUserName, Integer createTime,
		...
		//通讯录变更事件
		if ("change_contact".equalsIgnoreCase(event)) {
			String ChangeType = xmlHelper.getString("//ChangeType");
			if(InAddressOfficeEvent.EVENT_CREATE_PARTY.equalsIgnoreCase(ChangeType)) {
				return new InAddressOfficeEvent(xmlHelper,toUserName, fromUserName, createTime, msgType, event, agentId);
			}
			...
			if(InAddressUserEvent.EVENT_DELETE_USER.equalsIgnoreCase(ChangeType)){
				return new InAddressUserEvent(xmlHelper,toUserName, fromUserName, createTime, msgType, event, agentId);
			}
		...
		if ("subscribe".equalsIgnoreCase(event) && StringUtils.isNoneBlank(eventKey) && eventKey.startsWith("qrscene_")) {
