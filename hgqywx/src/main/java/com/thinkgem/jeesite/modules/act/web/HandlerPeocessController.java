...
@Controller
@RequestMapping(value = "${adminPath}/act/handlerpeocess")
...
@RequestMapping(value = "/withdrawTaskTo", method = RequestMethod.POST)
    public  String execute(String taskId,RedirectAttributes redirectAttributes,String view) {
        try {
            // 取得当前任务
          ...
          //根据流程id查询代办任务中流程信息
        	...
        	addMessage(redirectAttributes, "流程取回成功");
        } catch (Exception e) {
            addMessage(redirectAttributes, "流程取回失败，未知错误");
        }
		return view;
    }
             ...
             */  
             public void jointProcess(String taskId, List<String> userCodes)  
                 ...
                 for (String userCode : userCodes) {  
                     TaskEntity task = (TaskEntity) taskService.newTask(IdGen.uuid());  
