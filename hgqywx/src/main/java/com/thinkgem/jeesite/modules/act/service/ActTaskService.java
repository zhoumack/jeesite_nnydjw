...
public class ActTaskService extends BaseService {
	...
	public List<Act> todoList(Act act){
		...
		result = todoList(userId, act, result);
...
//		}
		Collections.sort(result, new Comparator<Act>() {
			...
			public int compare(Act a, Act b) {
				...
				return b.getTask().getCreateTime().compareTo(a.getTask().getCreateTime());
			}
	 ...
	 *
	 */
	public List<Act> todoListByUserId(Act act){
...
//		}else{
//			userId=UserUtils.get(act.getId()).getLoginName();
//		}
		userId=(StringUtils.isEmpty(act.getAssignee())?UserUtils.getUser().getLoginName():act.getAssignee());
		List<Act> result = new ArrayList<Act>();
...
//		}
	 ...
	 * @return
	 */
	public List<List<Act>> todoListOffice(Act act){
		User u = UserUtils.getUser();
		
		List<List<Act>> result = new ArrayList<List<Act>>();
		
		// 不是部门领导直接返回
		if(!"1".equals(this.wodeOfficeRole())) {
			return result;
		}
		
		String officeId = u.getOffice().getId();
		...
		// 循环查询所有用户的代办
		for(User user : users) {
			List<Act> l = todoList(user.getLoginName(), act, null);
	...
	public String wodeOfficeRole() {
		...
		if(sb.toString().contains("fgld") || sb.toString().contains("yld")
				|| sb.toString().contains("_ld") || sb.toString().contains("bmld")){
			return "1";
		}else {
			return "0";
		}
	...
	/**
	 * 查询代办
	 * @param userId 要查询代办的用户
	 * @param act
	 * @param result 返回结果，如果多次调用，结果将进行累加
	 * @return
	 */
	private List<Act> todoList(String userId, Act act, List<Act> result){
		if(result == null) {
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
			...
			System.out.println("已经签收的任务:"+taskDefinitionKey);
		}
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
			...
			result.add(e);
			String taskDefinitionKey = task.getTaskDefinitionKey();
			System.out.println("等待签收的任务:"+taskDefinitionKey);
		}
	...
	public Page<Act> historicList(Page<Act> page, Act act){
		...
		for (HistoricTaskInstance histTask : histList) {
			if(histTask.getTaskDefinitionKey().equals("audit")) continue;
			Act e = new Act();
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
			e.setStatus("finish");
			List<Task> currentTaskList = getCurrentTaskList(histTask.getProcessInstanceId());
			if(CollectionUtils.isEmpty(currentTaskList)){
				e.getVars().getMap().put("actStatus", "finish");
			}else{
				e.getVars().getMap().put("actStatus", "todo");
			}
			actList.add(e);
			//page.getList().add(e);
		}
	 ...
	 * @return
	 */
	public List<Act>  historicApply(Act act){

		HistoricTaskInstanceQuery histTaskQuery = historyService.createHistoricTaskInstanceQuery().taskAssignee(act.getAssignee()).finished()
				.includeProcessVariables().orderByHistoricTaskInstanceEndTime().desc();
...
//						　　string s = Encoding.UTF8.GetString(bb);
						//解析blob乱码
						try {
							if(StringUtils.getBytes(fullMessage)!=null) fullMessage= new String(fullMessage.getBytes(),"utf-8");
	 ...
	 * @return
	 */
	public List<HistoricProcessInstance> proInsHisList( String startUser,String proInsId) {
		HistoricProcessInstanceQuery hpq=historyService.createHistoricProcessInstanceQuery().finished().
		orderByProcessInstanceStartTime().desc();
		 if (StringUtils.isNotEmpty(startUser)){
			 hpq.startedBy(startUser);
		 }
		 if (StringUtils.isNotEmpty(proInsId)){
			 hpq.processInstanceId(proInsId);
		 }
		 //List<HistoricProcessInstance> processDefinitionList = hpq.listPage(page.getFirstResult(), page.getMaxResults());
		 List<HistoricProcessInstance> hishpq=hpq.list();
		 return hishpq;
	}
	/**
	 * 获取流程列表
	...
	public HistoricProcessInstance getHisProcIns(String procInsId) {
		...
		HistoricProcessInstance hisIns = historyService.createHistoricProcessInstanceQuery().processInstanceId(procInsId).singleResult();
		return hisIns;
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public void addComment(String taskId, String procInsId, String comment){
...
//		assigneeList.add(Authentication.getAuthenticatedUserId());
	...
	@Transactional(readOnly = false)
	@SuppressWarnings("unchecked")
	public ActivityImpl[] insertTasksAfter2(String procDefId, String procInsId, String targetTaskDefinitionKey, Map<String, Object> variables, String... assignees) {
		List<String> assigneeList = new ArrayList<String>();
		assigneeList.add(Authentication.getAuthenticatedUserId());
		assigneeList.addAll(CollectionUtils.arrayToList(assignees));
		String[] newAssignees = assigneeList.toArray(new String[0]);
		ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity)repositoryService.getProcessDefinition(procDefId);
//		ActivityImpl prototypeActivity = ProcessDefUtils.getActivity(processEngine, processDefinition.getId(), targetTaskDefinitionKey);
		return cloneAndMakeChain(processDefinition, procInsId, targetTaskDefinitionKey, targetTaskDefinitionKey, variables, newAssignees);
	}
	/**
	 * 前加签
	 */
...
//		recordActivitiesCreation(info);
	...
	public List<Task> getCurrentTaskList(String procInsId) {
		return (List<Task>) taskService.createTaskQuery().processInstanceId(procInsId).active().list();
	}
	@Transactional(readOnly = false)
...
//		return ProcessDiagramGenerator.generateDiagram(bpmnModel, "png", activeActivityIds);
	 ...
	 * @param candidateGroup 角色组
	 * @param taskDefinitionKey   节点定义名称
	 * @return
	 */
	public List<HistoricTaskInstance> histoicList(String procInsId,String candidateGroup,String taskDefinitionKey){
		HistoricTaskInstanceQuery histTaskQuery =historyService.createHistoricTaskInstanceQuery().processInstanceId(procInsId).includeProcessVariables().orderByHistoricTaskInstanceEndTime().desc();
		// 设置查询条件
				if (StringUtils.isNotBlank(candidateGroup)){
					histTaskQuery.taskCandidateGroup(candidateGroup);
				}
				if (StringUtils.isNotBlank(taskDefinitionKey)){
					histTaskQuery.taskDefinitionKeyLike(taskDefinitionKey);
				}
		List<HistoricTaskInstance> histList = histTaskQuery.list();
		return histList;
	}
