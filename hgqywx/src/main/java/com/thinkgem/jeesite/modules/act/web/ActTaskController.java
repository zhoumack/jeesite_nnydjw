...
public class ActTaskController extends BaseController {
	 ...
	 */
	@RequestMapping(value = "histoicFlowData")
	public String histoicFlowData(Act act, String startAct, String endAct, Model model,HttpServletRequest request){
		String view="modules/act/histoicMobileFlowList_2";
	...
	public String histoicFlow(Act act, String startAct, String endAct, Model model,HttpServletRequest request){
		String view="modules/act/actTaskHistoricFlow";
		if (StringUtils.isNotBlank(act.getProcInsId())){
			List<Act> histoicFlowList = actTaskService.histoicFlowList(act.getProcInsId(), startAct, endAct);
			model.addAttribute("histoicFlowList", histoicFlowList);
		}
		...
		if (principal.isMobileLogin()){
			view="modules/act/histoicMobileFlowList";
		}
		return view;
	}
	/**
	 ...
	 * @param category 流程分类
	 */
	@RequestMapping(value = "proInsHisList")
	public String proInsHisList(String proInsId, HttpServletRequest request, HttpServletResponse response, Model model) {
	    List<HistoricProcessInstance>hisProIns = actTaskService.proInsHisList(UserUtils.getUser().getLoginName(), proInsId);
		model.addAttribute("hisProIns", hisProIns);
		//model.addAttribute("category", category);
		return "modules/act/actTaskProcessList";
	}
	/**
	...
	public String form(Act act, HttpServletRequest request, Model model){
		...
		String busId=act.getBusinessId();
		if(StringUtils.isEmpty(busId)){
			HistoricProcessInstance hisIns=actTaskService.getHisProcIns(act.getProcInsId());
			if (hisIns != null && hisIns.getBusinessKey() != null){
				String[] ss = hisIns.getBusinessKey().split(":");
