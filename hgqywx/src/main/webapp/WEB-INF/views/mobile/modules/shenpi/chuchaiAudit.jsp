...
<!--内容  -->
  <div class="center_dv">
		<div class="center_dv_dv" style="float: left">
			...
			<span>${chuchai.user.name}的出差申请</span>
		</div>
		...
		<div class="center_dv_dv2">
			<div class="center_span">
				<span>申请姓名</span> <span class="center_span_s2">${chuchai.user.name}</span>
			</div>
			<div class="center_span">
				<span>出差地点</span> <span class="center_span_s2">${chuchai.direction}</span>
			</div>
			<div class="center_span">
				<span>开始时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${chuchai.starttime}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
				<span>结束时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${chuchai.endtime}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
				<span>出差时长</span> <span class="center_span_s2">${chuchai.totaldays}天 </span>
			</div>
			<div class="center_span">
				<span>出差事由</span>
				<%-- <span class="center_span_s2">${chuchai.event}</span> --%>
				<p  style="width: 70%;float: right;font-size: 16px;color: #000">${chuchai.event==""?"无":chuchai.event }</p>
				<br style="clear: both;">
			</div>
			<div class="center_span">
				<span>说明附件</span> <span class="center_span_s2">${chuchai.filepath==""?"无":" " }</span>
				<div style="margin-top: 10px">
					<c:forEach items="${chuchai.filepath.split(',') }" var="img" varStatus="status">
						<c:if test="${!empty img}">
  ...
  <!--审批流程  -->
  	<form:form id="inputForm" modelAttribute="chuchai" action="${ctx}/shenpi/chuchai/saveAudit" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<form:hidden id="flag" path="act.flag"/>
  		<div id="spyijian">
  		<div class="liu_dv">
		<act:histoicMobileFlowList_2 procInsId="${chuchai.procInsId}" />
		<div class="xian"></div>
		<c:if test="${!empty chuchai.copy }">
			<act:histoicMobileFlowList_3 procInsId="${chuchai.copy}" />
		</c:if  >
		<c:if test="${empty chuchai.copy }">
    ...
    <div id="half" class="weui-popup__container" >
        ...
        <div class="weui-popup__modal" id="app">
            ...
            <input type="hidden" id="approve" name="approve" value="${chuchai.approve }"/>
        </div>
    </div>
    <!--底部导航-->
		<div class="footer_op">
