...
.js_xqing{
width: 100%;height: 200px;background-color: #fff;margin-top: 20px
}
.js_xqing p{
...
<%@include file="/WEB-INF/views/include/mmenu.jsp" %>
	...
	<form:form id="inputForm" modelAttribute="paiche" action="${ctx}/shenpi/paiche/saveAudit" method="post" class="form-horizontal">
		...
		<!-- 申请部门领导人审批 -->
		<c:if test="${jiedian=='audit'}">
			<div id="baozhang" class="t_row">
				<div class="row_div">
					<div>部门领导<span class="bt_span">*</span></div>
		...
		<!-- 分管领导 -->
		<c:if test="${jiedian=='fgldsp'}"></c:if>
		
		<!-- 部门领导 -->
		<c:if test="${jiedian=='bmldrsp'}">
			<c:if test="${paiche.iscommon=='0'}">
			<script>
			$(function(){
				$(document).ready(function() {
					setTimeout(function(){resetData("${ctx}/sys/role/treejson1?renname=fgld");},1000);
				});
			});
			</script>
				<div id="baozhang" class="t_row">
					<div class="row_div">
						<div>分管领导<span class="bt_span">*</span></div>
						<div class="t_ta_div mui-row mui-col-xs-12 sel_top_div">
							<div class="mui-col-xs-3 sel_a_div">
								<a id="spr_a" class="sel_a" href="#menu" sel_type="csr" onclick="sel_click(this)"></a>
								<input  id="fgld" name="taskApprove" type="hidden" value='' />
							</div>
						</div>
					</div>
				</div>
		...
		<!-- 驾驶员分配 -->
		<c:if test="${jiedian=='nqfpjsy'}">
			<div id="baozhang" class="t_row">
				<div class="row_div">
					<div>驾驶员分配<span class="bt_span">*</span></div>
					<div class="t_ta_div mui-row mui-col-xs-12 sel_top_div">
						<div class="mui-col-xs-3 sel_a_div">
							<a class="sel_a" href="#menu" sel_type="csr" onclick="sel_click(this)"></a>
							<input  id="jsyfp" name="taskApprove" type="hidden" value='' />
						</div>
					</div>
				</div>
			</div>
		...
		<div class="footer_op">
			...
			<div class="mui-row mui-col-xs-12">
				<button class="mui-col-xs-6" type="button" id="tongyi"
					onclick="sub_form('1')">
					<img src="${ctxStatic}/mui/img/tongyi.png" width="20px"
						height="20px" style="position: relative; top: 5px" /> 同意
				</button>
				<button class="mui-col-xs-6" type="button" id="jujue"
					onclick="sub_form('2');">
					<img src="${ctxStatic}/mui/img/bohui.png" width="20px"
						height="20px" style="position: relative; top: 5px" /> 不同意
				</button>
			</div>
		</div>
	</form:form>
	...
	function sub_vail() { 
		var spr = $("#spr").val();
		var fgld = $.trim($("#fgld").val());
		var jsyfp = $.trim($("#jsyfp").val());
		var fgld1 = '${jiedian}';
		var fgld2='${paiche.iscommon}';
		
		if (fgld1 == 'bmldrsp' && fgld2 == '0') {
			if (fgld) {
				var fgld_obj = JSON.parse(fgld); 
