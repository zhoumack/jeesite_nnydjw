...
<!-- 悬浮 -->
	...
	<%-- <form:form id="inputForm" modelAttribute="renwu" action="${ctx}/shenpi/renwu/save" method="post" class="form-horizontal"> --%>
	<div class="center_dv">
		<div class="center_dv_dv" style="float: left">
			<img src="${ctxStatic}/mui/img/header.jpg" width="30px" height="30px"
				style="border-radius: 5px" /> <span>${renwu.user.name}的任务安排</span>
		</div>
		<div class="sp_dv" style="float: left">
			<span id="shenpi">${renwu.status }</span>
		</div>
		<div style="clear: both;"></div>

		<div class="center_dv_dv2" style="border-bottom: 1px solid #e5e5e5;">
			...
			<div class="center_span">
				<span>结束时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${renwu.endtime}" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
		...
		<c:forEach items="${bmldfprw}" var="zxr">
			<div class="center_dv_dv2">
				<div class="center_span">
					<span>执&nbsp;&nbsp;行&nbsp;人</span> <span class="center_span_s2">${zxr.user.name}</span>
				</div>
				<div class="center_span">
					<span>完成时间</span> <span class="center_span_s2">${zxr.endTime}</span>
				</div>
				<div class="center_span">
					<span>完成状态</span> <span class="center_span_s2">${zxr.commont}</span>
				</div>
				<div class="center_span">
					<span>工作反馈</span> <span class="center_span_s2">${zxr.remark}</span>
				</div>
			</div>
		</c:forEach>
	</div>
	...
	<act:histoicMobileFlowList procInsId="${renwu.procInsId}" />
	<%-- </form:form> --%>
	</div>
	</div>
