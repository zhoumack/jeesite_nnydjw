...
<body>
	...
	<!--内容  -->
	<form:form id="inputForm" modelAttribute="qingjia"
		...
		<div class="center_dv">
			<div class="center_dv_dv" style="float: left">
				<img src="${ctxStatic}/mui/img/header.jpg" width="30px"
					height="30px" style="border-radius: 5px" /> <span>${qingjia.user.name }的请假申请</span>
			</div>
			<div class="sp_dv" style="float: left">
				<span id="shenpi">${qingjia.status }</span>
			</div>
			...
			<div class="center_dv_dv2">
				...
				<div class="center_span">
					<span>请假类型</span> <span class="center_span_s2">${qjType}</span>
				</div>
				<div class="center_span">
				...
				<div class="center_span">
					...
					<div style="margin-top: 10px">
						<c:forEach items="${qingjia.filepath.split(',') }" var="img"
							varStatus="status">
							<c:if test="${!empty img}">
		...
		<div id="spyijian" style="display: none;">
			<div class="weui-cell weui_footer1">
				<input class="weui-input" type="hidden" name="act.comment"
					value="${qingjia.remarks }">
			</div>
		</div>
		<act:histoicMobileFlowList procInsId="${qingjia.procInsId}" />
	</form:form>
	<script>
		mui.previewImage();
