...
<!--内容  -->
<form:form id="inputForm" modelAttribute="yongjing" action="${ctx}/shenpi/yongjing/save" method="post" class="form-horizontal">
	<div class="center_dv">
		<div class="center_dv_dv" style="float: left">
			...
			<span>${yongjing.user.name}的用警申请</span>
		</div>
		<div class="sp_dv" style="float: left">
			<span id="shenpi">${yongjing.status }</span>
		</div>
		<div style="clear: both;"></div>

		<div class="center_dv_dv2">
			<div class="center_span">
				<span>申请姓名</span> <span class="center_span_s2">${yongjing.user.name}</span>
			</div>
			<div class="center_span">
				<span>用警类型</span> <span class="center_span_s2">${yongjing.type}</span>
			</div>
			<div class="center_span">
				<span>出警地点</span> <span class="center_span_s2">${yongjing.place}</span>
			</div>
			<div class="center_span">
				<span>出警时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${yongjing.time}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
				<span>出警事由</span> <span class="center_span_s2">${yongjing.event}</span>
			</div>
			<div class="center_span">
				<span>说明附件</span> <span class="center_span_s2">${yongjing.filepath==""?"无":" " }</span>
				<div style="margin-top: 10px">
					<c:forEach items="${yongjing.filepath.split(',') }" var="img" varStatus="status">
						<c:if test="${!empty img}">
  	...
  	<div id="spyijian" style="display: none;">
	      ...
	      <input class="weui-input" type="hidden" name="act.comment"  value="${yongjing.remarks }">
		</div>
  	...
  	<act:histoicMobileFlowList procInsId="${yongjing.procInsId}" />
  	</form:form>
