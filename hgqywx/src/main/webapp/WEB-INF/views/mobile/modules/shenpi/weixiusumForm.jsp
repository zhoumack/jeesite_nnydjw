...
<form:form id="inputForm" modelAttribute="weixiu" action="${ctx}/shenpi/weixiu/save" method="post" class="form-horizontal">
	<div class="center_dv">
		...
		<div class="center_dv_dv2">
			...
			<c:choose >
					<c:when test="${weixiu.cost != null }">
					    <div class="center_span">
								<span>涉及金额</span> <span class="center_span_s2">
									${weixiu.cost}
								</span>
						</div>
					</c:when>
			...
			<c:choose >
					<c:when test="${weixiu.donedate != null }">
					    <div class="center_span">
							<span>完成时间</span> <span class="center_span_s2"><fmt:formatDate
									value="${weixiu.donedate}" type="both"
			...
			<c:choose >
					<c:when test="${weixiu.remarks != null }">
					   <div class="center_span">
							<span>维修意见</span> <span class="center_span_s2">${weixiu.remarks}</span>
						</div>
					</c:when>
			</c:choose>
			<c:choose >
					<c:when test="${weixiu.result != null }">
					    <div class="center_span">
							<span>维修结果</span> <span class="center_span_s2">${weixiu.result}</span>
						</div>
					</c:when>
			</c:choose>
			<div class="center_span">
