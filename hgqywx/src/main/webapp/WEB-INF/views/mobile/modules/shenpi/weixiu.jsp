...
<style type="text/css">
	...
	function openPicker(ele) {
		...
		pop_picker.show(function(items) {
			...
			var user_id = "${weixiu.user.id}";
			$("#wxsxly_text").val(items[0].text);
			$("#wxsxly_val").val(items[0].value);
			pop_picker.dispose();
		});
	}
			...
			function add_spr(dataId, dataType, dataImg, dataName) {
				...
				if($(sel_a_obj).parents('.sel_a_div').find('#userName').length == 0){//如果不是审批人
			...
			function sub_vail() {
				...
				if (wxsxly_val == "2") {
					 ...
					 if (userName) {
							var fgld_obj = JSON.parse(userName); 
...
<body>
	...
	<img id="rc_btn" src="${ctxStatic}/mui/image/rc_more.png"
		style="z-index: 999; position: absolute; width: 40px; right: 17px; bottom: 138px">
	<div id="poper_m"></div>
	<div id="poper_dv">
	...
	<%@include file="/WEB-INF/views/include/mmenu.jsp" %>
			...
			<div class="div_body">
				<form:form id="qj_form" action="${ctx}/shenpi/weixiu/save" modelAttribute="weixiu" method="post" onsubmit="sub_form(this);return false;">
					<form:hidden path="id"/>
					...
					<div class="t_row">
					 <div class="row_div">
							<a class="t_a" href="#">
								...
								<div class="t_div">
									<input id="wxsxly_val" name="esource"  class="t_input" value="${weixiu.user.office.name }-${weixiu.user.name }" readonly="readonly" />
								</div>
							</a>
						</div>
						<!-- <div class="row_div"> -->
<%-- 							<shiro:hasRole name="jwbzb_nq"> --%>
<!-- 							<a class="t_a" onclick="openPicker(this)" href="#"> -->
<!-- 								<span class="t_span">事项来源<span class="bt_span">*</span></span> -->
<!-- 								<div class="t_div"> -->
<!-- 									<input id="wxsxly_text"   placeholder="请选择" class="t_input" type="text" readonly="readonly" /> -->
<!-- 									<input id="wxsxly_val"   type="hidden" name="esource" /> -->
<!-- 										<input id="wxsxly_val" name="esource" > -->
<!-- 								</div> -->
<!-- 							</a> -->
<%-- 							</shiro:hasRole> --%>
							<%-- <shiro:lacksRole name="jwbzb_nq"> --%>
							<%-- <span class="t_span">事项来源<span class="bt_span">*</span></span>
							<div>
<!-- 							<input id="wxsxly_text" value="部门移送" class="t_input" type="text" readonly="readonly" /> -->
								<input id="wxsxly_val"  class="t_input" name="esource" value="${weixiu.user.office.name }-${weixiu.user.name }" readonly="readonly"/>
							</div> --%>
							<%-- </shiro:lacksRole>  --%>
						<!-- </div> -->
					</div>
<%-- 					<shiro:hasRole name="jwbzb_nq"> --%>
<!-- 					<div id="shenqingr" class="t_row" style="display: none;"> -->
<!-- 						<div class="row_div"> -->
<!-- 							<div>申请人<span class="bt_span">*</span></div> -->
<!-- 							<div class="t_ta_div mui-row mui-col-xs-12 sel_top_div"> -->
<!-- 								<div class="mui-col-xs-3 sel_a_div"> -->
<!-- 									<a id="spr_a" class="sel_a" href="#menu" sel_type="spr" onclick="sel_click(this)"></a> -->
...
<%-- 					</shiro:hasRole> --%>
					...
					<div class="t_row"> 
						<div class="row_div">
							<a class="t_a" href="#">
								<span class="t_span">故障设备<span class="bt_span">*</span></span>
								<div class="t_div">
									<input id="gzsb" placeholder="请输入" class="t_input" type="text" name="item" />
								</div>
							</a>
						</div>
					</div>
					...
					<div class="t_row">
						<div class="row_div">
							<a class="t_a" onclick="openTimePicker(this)" href="#">
								<span class="t_span">报修时间<span class="bt_span">*</span></span>
								<div class="t_div">
									<input id="starttime" type="text" placeholder="请选择" class="t_input" readonly="readonly" />
									<input type="hidden" id="starttime_hid" name="date">
								</div>
							</a>
						</div>
					</div>
					...
					<div class="t_row">
						<div class="row_div">
							<div>故障描述</div>
							<div class="t_ta_div">
								<textarea name="event" placeholder="请输入故障描述" class="t_textarea" type="text"></textarea>
							</div>
						</div>
					</div>
...
function load_uploader() {
	...
	$(document).on('tap','.uploader_del',function(){
		...
		//dom元素删除
		parent.remove();
		if($("#userName").parents('.sel_top_div').find('.sel_div').length == 0){
