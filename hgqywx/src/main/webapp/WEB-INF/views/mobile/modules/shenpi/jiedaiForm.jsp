...
<!--内容  -->
<form:form id="inputForm" modelAttribute="jiedai" action="${ctx}/shenpi/jiedai/save" method="post" class="form-horizontal">
	<div class="center_dv">
		<div class="center_dv_dv" style="float: left">
			...
			<span>${jiedai.user.name}的接待申请</span>
		</div>
		<div class="sp_dv" style="float: left">
				<span id="shenpi">${jiedai.status}</span>
			</div>
		<div style="clear: both;"></div>

		<div class="center_dv_dv2">
			<div class="center_span">
				<span>申请姓名</span> <span class="center_span_s2">${jiedai.user.name}</span>
			</div>
			<div class="center_span">
				<span>负&nbsp;&nbsp;责&nbsp;人</span> <span class="center_span_s2">${jiedai.charge}</span>
			</div>
			<div class="center_span">
				<span>接待单位</span> <span class="center_span_s2">${jiedai.unit}</span>
			</div>
			<div class="center_span">
				<span>接待人数</span> <span class="center_span_s2">${jiedai.pnums}人</span>
			</div>
			<div class="center_span">
				<span>接待时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${jiedai.starttime}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<%-- <div class="center_span">
				<span>结束时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${jiedai.endtime}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div> --%>
			<div class="center_span">
				<span>会议安排</span> <span class="center_span_s2">${jiedai.meetingplan}</span>
			</div>
			<div class="center_span">
				<span>用餐安排</span> <span class="center_span_s2">${jiedai.dinnerplan}</span>
			</div>
			<div class="center_span">
				<span>用车安排</span> <span class="center_span_s2">${jiedai.carplan}</span>
			</div>
			<div class="center_span">
				<span>说明附件</span> <span class="center_span_s2">${jiedai.filepath==""?"无":" " }</span>
				<div style="margin-top: 10px">
					<c:forEach items="${jiedai.filepath.split(',') }" var="img" varStatus="status">
						<c:if test="${!empty img}">
  	...
  	<div id="spyijian" style="display: none;">
	      ...
	      <input class="weui-input" type="hidden" name="act.comment"  value="${jiedai.remarks }">
		</div>
  	...
  	<act:histoicMobileFlowList procInsId="${jiedai.procInsId}" />
  	</form:form>
