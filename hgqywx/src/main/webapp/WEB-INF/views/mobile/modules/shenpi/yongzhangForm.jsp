...
<form:form id="inputForm" modelAttribute="yongzhang" action="${ctx}/shenpi/yongzhang/save" method="post" class="form-horizontal">
	<div class="center_dv">
		<div class="center_dv_dv" style="float: left">
			...
			<span>${yongzhang.user.name}的用章申请</span>
		</div>
		<div class="sp_dv" style="float: left">
			<span id="shenpi">${yongzhang.status }</span>
		</div>
		<div style="clear: both;"></div>

		<div class="center_dv_dv2">
			<div class="center_span">
				<span>申请姓名</span> <span class="center_span_s2">${yongzhang.user.name}</span>
			</div>
			<div class="center_span">
				<span>用章类型</span> <span class="center_span_s2">${yongzhang.type}</span>
			</div>
			<div class="center_span">
				<span>文件名称</span> <span class="center_span_s2">${yongzhang.filename}</span>
			</div>
			<div class="center_span">
				<span>文件份数</span> <span class="center_span_s2">${yongzhang.filenums}</span>
			</div>
			<div class="center_span">
				<span>存档份数</span> <span class="center_span_s2">${yongzhang.savenums}</span>
			</div>
			<div class="center_span">
				<span>用章时间</span> <span class="center_span_s2">
				<fmt:formatDate value="${yongzhang.date}" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
				<span>用章事由</span> <span class="center_span_s2">${yongzhang.event }</span>
			</div>
			<div class="center_span">
				<span>说明附件</span> <span class="center_span_s2">${yongzhang.filepath==""?"无":" " }</span>
				<div style="margin-top: 10px">
					<c:forEach items="${yongzhang.filepath.split(',') }" var="img" varStatus="status">
						<c:if test="${!empty img}">
  	...
  	<div id="spyijian" style="display: none;">
	      ...
	      <input class="weui-input" type="hidden" name="act.comment"  value="${yongzhang.remarks }">
		</div>
  	...
  	<act:histoicMobileFlowList procInsId="${yongzhang.procInsId}" />
  	</form:form>
