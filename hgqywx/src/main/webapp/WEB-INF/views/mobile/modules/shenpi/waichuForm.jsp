...
<form:form id="inputForm" modelAttribute="waichu" action="${ctx}/shenpi/waichu/save" method="post" class="form-horizontal">
	<div class="center_dv">
		<div class="center_dv_dv" style="float: left">
			...
			<span>${waichu.user.name}的外出申请</span>
		</div>
		<div class="sp_dv" style="float: left">
			<span id="shenpi">${waichu.status }</span>
		</div>
		<div style="clear: both;"></div>

		<div class="center_dv_dv2">
			<div class="center_span">
				<span>申请姓名</span> <span class="center_span_s2">${waichu.user.name}</span>
			</div>
			<div class="center_span">
				<span>外出地点</span> <span class="center_span_s2">${waichu.place}</span>
			</div>
			<div class="center_span">
				<span>开始时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${waichu.starttime}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
				<span>结束时间</span> <span class="center_span_s2"><fmt:formatDate
						value="${waichu.endtime}" type="both"
						pattern="yyyy-MM-dd HH:mm:ss" /></span>
			</div>
			<div class="center_span">
				<span>外出时长</span> <span class="center_span_s2">${waichu.totaltime}小时</span>
			</div>
			<div class="center_span">
				<span>外出事由</span> <span class="center_span_s2">${waichu.event}</span>
			</div>
			<div class="center_span">
				<span>说明附件</span> <span class="center_span_s2">${waichu.filepath==""?"无":" " }</span>
				<div style="margin-top: 10px">
					<c:forEach items="${waichu.filepath.split(',') }" var="img" varStatus="status">
						<c:if test="${!empty img}">
  	...
  	<div id="spyijian" style="display: none;">
	      ...
	      <input class="weui-input" type="hidden" name="act.comment"  value="${waichu.remarks }">
		</div>
  	...
  	<act:histoicMobileFlowList procInsId="${waichu.procInsId}" />
  	</form:form>
