...
<form:form id="inputForm" modelAttribute="paiche" action="${ctx}/shenpi/paiche/save" method="post" class="form-horizontal">
				 ...
				 <c:forEach items="${jsyxx}" var="sjxx">
				 	<li class="mui-table-view-cell mui-collapse">
						...
						<div class="mui-collapse-content">
							...
							<div class="center_span">
								<span>联系电话</span> <span class="center_span_s2"><a href="tel:${sjxx.driverPhone}">${sjxx.driverPhone }</a></span>
							</div>
							<c:choose >
									<c:when test="${sjxx.hysj != null }">
									   <div class="center_span">
											<span>回院时间</span> 
											<span class="center_span_s2 hysj">${sjxx.hysj }</span>
										</div>
									</c:when>
							</c:choose>
	...
	mui.previewImage();
		...
		var _temp=$(".hysj").html();
		if(_temp!=''&&_temp!='undefind'&&_temp!=null)
			{
				$(".hysj").html(_temp.substring(0,10));
			}
