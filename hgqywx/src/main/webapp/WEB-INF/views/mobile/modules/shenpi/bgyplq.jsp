...
<script type="text/javascript">
			...
			$(document).ready(function() {
				var _url = "${ctx}/store/hcpj/findHcpj";
				$.getJSON(_url, {
					/* type: 'bgyplqType' */
				}, function(rsp) {
					if(rsp) {
						for(var i in rsp) {
							rsp[i].text = rsp[i].name;
							if(!rsp[i].children){
								rsp[i].children = [];
							}else{
								for(var j in rsp[i].children){
									rsp[i].children[j].text = rsp[i].children[j].name;
								}
							}
						}
						options = rsp;
			...
			function openPicker(ele) {
				var pop_picker = new mui.PopPicker({layer: 2});
				pop_picker.setData(options);
				pop_picker.show(function(items) {
					var one = items[0].value;
					var two = items[1].value;
					var three = items[1].nums;
					var four = items[1].id;
					$("#qjsy_input").val(items[1].nums);
					$("#qjlx_text").val(items[0].text +" "+ items[1].text);
					
					//$("#qjlx_val").val(items[0].value +" "+ items[1].value);
					
					$("#qjlx_val").val(items[0].text +" "+ items[1].text);
					console.log(four);
					$("#qjlx_id").val(four);
					console.log('aaa='+$("#qjlx_id").val());
				});
			} 
			...
			function sub_vail() {
				...
				if(!isNaN(itemnums)){
					if(parseFloat(itemnums) > parseFloat(qjsy)){
						mui.toast('申请数量不能大于库存！');
						return false;
					}
				}
...
<img id="rc_btn" src="${ctxStatic}/mui/image/rc_more.png" style="z-index: 999; position: absolute; width: 40px; right: 17px; bottom: 154px;">
	<div id="poper_m"></div>
	<div id="poper_dv">
...
<!--内容  -->
	...
	<div class="div_body">
		<form:form id="_form" action="${ctx}/shenpi/bgyplq/save"
			modelAttribute="bgyplq" method="post"
			...
			<div class="t_row">
				<div class="row_div">
						<span class="t_span">事项来源<span class="bt_span">*</span></span>
							<div>
								<input id="wxsxly_val"  class="t_input" name="esource" value="${bgyplq.user.office.name }-${bgyplq.user.name }" readonly="readonly"/>
							</div>
				</div>
				
				<div class="row_div">
							<a class="t_a" href="#">
								<span class="t_span">房间号<span class="bt_span">*</span></span>
								<div class="t_div">
									<input id="houseNumber" name="houseNumber" placeholder="请输入" class="t_input" type="text" />
								</div>
							</a>
				</div>
				...
				<div class="row_div">
					<a class="t_a" onclick="openPicker(this)" href="#">
						<span class="t_span">物品选择<span class="bt_span">*</span></span>
						<div class="t_div">
							<input id="qjlx_text" placeholder="请选择" class="t_input" type="text" readonly="readonly" />
							<input id="qjlx_val" type="hidden" name="item" />
							<input style="display: none;" id="qjlx_id" type="hidden" name="bgypId" />
						</div>
					</a>
				</div>
				<div class="row_div" style="display: none;">
					<a class="t_a" href="#">
						<span class="t_span">仓库剩余</span>
						<div class="t_div" style="padding-right: 20px;position: relative;">
							...
							<span style="position: absolute;top:0;right: 0;color: gray;">件</span>
						</div>
					</a>
				</div>
			</div>
			<div class="t_row">
				...
				<a class="t_a" onclick="openTimePicker(this)" href="#">
					<span class="t_span">申请时间<span class="bt_span">*</span></span>
					<div class="t_div">
						<input id="date" type="text" name="date" placeholder="请选择" class="t_input" readonly="readonly" />
					</div>
				</a>
			</div>
			<div class="row_div">
				<a class="t_a" href="#"> <span class="t_span">数量<span
						class="bt_span">*</span></span>
					<div class="t_div">
						<input id="itemnums" placeholder="数量" class="t_input" type="number"
							name="itemnums" />
					</div>
				</a>
			</div>
		...
		<div class="t_row">
				<div class="row_div">
					<div>
						用途 <span class="bt_span">*</span>
					</div>
					<div class="t_ta_div">
						<textarea id="usefor" name="usefor" placeholder="请输入"
	...
	<!-- <div class="t_row">
		<div class="row_div">
			<div>
				选部门人<span class="bt_span">*</span>
			</div>
			<div class="t_ta_div mui-row mui-col-xs-12 sel_top_div">
				<div class="mui-col-xs-3 sel_a_div">
					<a id="spr_a" class="sel_a" href="#menu" sel_type="spr" onclick="sel_click(this)"></a> <input id="spr" name="approve" type="hidden" value='' />
				</div>
			</div>
		</div>
	</div> -->
	...
	<div class="t_row">
		<div class="row_div">
			...
			<div class="t_ta_div mui-row mui-col-xs-12 sel_top_div">
				<div class="mui-col-xs-3 sel_a_div">
					<a class="sel_a" href="#menu" sel_type="csr"
						onclick="sel_click(this)">
					</a>
